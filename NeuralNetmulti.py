...
class NeuralNetwork:
	...
	def __init__(self, layerSize):
		...
		temp = genfromtxt('sound_attributes_mfccShort.csv', delimiter=',')
		self.attributes = temp[:, range(temp.shape[1])]

		temp2 = genfromtxt('testing_attributes_mfccShort.csv', delimiter=',')
		self.testingAttributes = temp2[:, range(temp2.shape[1])]

		temp3 = genfromtxt('sound_labels_mfccShort.csv', delimiter=',')
		self.labels = np.reshape(temp3, (-1, 1))

		temp4 = genfromtxt('testing_labels_mfccShort.csv', delimiter=',')
		self.testingLabels = np.reshape(temp4, (-1, 1))

		#this should make an array of arrays with a 1 at index of label and 0 elsewhere to use to compare to guess
		for i in self.labels:
			x = [0]*SIZE
			x[int(i)] = 1
			self.ev.append(x)

		for i in self.testingLabels:
			y = [0]*SIZE
			y[int(i)] = 1
			self.evTest.append(y)
	...
	def Run(self):
		...
		for i in range(self.iterations):
			...
			#print self.labels, "hey", self.guesses[self.numLayers]

			#this calculates error, changed it to try to calculate error correctly
			errors[0] = self.ev - self.guesses[self.numLayers]
	...
	def Test(self, printAll=True):
		...
		for i in range(self.testingLabels.shape[0]):
			#print self.testingLabels[i], np.around(nextLayer , 0)
			print self.testingLabels[i], np.argmax(nextLayer[i])
			#mark -  this is supposed to find highest weighted output node and compare index to label
			if np.argmax(nextLayer[i]) == self.testingLabels[i]:
			#if self.testingLabels[i] == np.argmax(np.around(nextLayer, 0)[i]):
				numCorrect += 1
...
if __name__ == "__main__":
	...
	NN = NeuralNetwork((1326,1326,SIZE))
	#1326,1326
	NN.Run()
	# NN.Test()
